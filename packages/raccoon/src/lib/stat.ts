...
export const recommendFor = function(client: Redis, className: string, userId: string, numberOfRecs: number) {
...
export const recommendForWithScores = async function(
	...
	const recs = await client.zrevrange(recommendedZSetKey(className, userId), 0, numberOfRecs, 'WITHSCORES');

	return formatWithScoresResult(recs);
};

export const bestRated = function(client: Redis, className: string, limit: number) {
	return client.zrevrange(scoreboardZSetKey(className), 0, limit);
};
export const worstRated = function(client: Redis, className: string, limit = -1) {
	return client.zrange(scoreboardZSetKey(className), 0, limit);
};
export const bestRatedWithScores = async function(client: Redis, className: string, numOfRatings: number) {
	const ratings = await client.zrevrange(scoreboardZSetKey(className), 0, numOfRatings, 'WITHSCORES');
...
export const usersWhoLikedAlsoLiked = function(client: Redis, className: string, itemId: string) {
	console.log(itemId);
...
export const allWatchedFor = function(client: Redis, className: string, userId: string) {
	return client.sunion(userLikedSetKey(className, userId), userDislikedSetKey(className, userId));
};
