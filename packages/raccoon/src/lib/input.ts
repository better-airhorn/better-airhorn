...
import { updateSimilarityFor, updateWilsonScore, updateRecommendationsFor } from './algorithms';
import { Redis } from 'ioredis';
import Config from './config';

const updateSequence = async function(client: Redis, config: Config, userId: string, itemId: string) {
	...
	return Promise.all([
		...
		updateRecommendationsFor(client, config.className, config.nearestNeighbors, config.numOfRecsStore, userId),
	]);
};

export interface UpdateRecsOptions {
	updateRecs?: boolean;
}
...
const changeRating = async function(
	...
	if (options.updateRecs !== undefined) {
		updateRecommendations = Boolean(options.updateRecs);
	}

	const removeRating = Boolean(options.removeRating);
	...
	const mostFeelingSet = options.liked ? mostLikedKey(config.className) : mostDislikedKey(config.className);
	...
	removeRating ? await client.srem(feelingUserSet, itemId) : await client.sadd(feelingUserSet, itemId);
	removeRating ? await client.srem(feelingItemSet, userId) : await client.sadd(feelingItemSet, userId);
