...
const jaccardCoefficient = async function(client: Redis, className: string, userId1: string, userId2: string) {
	// finalJaccard = 0,
...
export const updateSimilarityFor = async function(client: Redis, className: string, userId: string) {
...
export const similaritySum = async function(client: Redis, simSet: string, compSet: string) {
	...
	await pMap(
		...
		async userId => {
			...
			const newScore = parseFloat(zScore as string) || 0.0;
			similarSum += newScore;
		},
		{ concurrency: 1 },
	);
...
export const updateWilsonScore = async function(client: Redis, className: string, itemId: string) {
